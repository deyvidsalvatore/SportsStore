...
export class Cart {
    ...
    constructor() {
        ...
        this.summary = computed(() => {
            ...
            this.linesSignal().forEach(i => {
                newSummary.itemCount += i.quantity;
                newSummary.cartPrice += i.lineTotal;
            });
            return newSummary;
        });
    }
    ...
    addLine(product: Product, quantity: number = 1) {
        this.linesSignal.mutate(linesArray => {
            let line = linesArray.find(i => i.product.id == product.id);
            if (line != undefined) line.quantity += quantity;
            else linesArray.push(new CartLine(product, quantity));
        });
    }
    ...
    removeLine(id: number) {
        this.linesSignal.mutate(linesArray => {
            let index = linesArray.findIndex(i => i.product.id == id);
